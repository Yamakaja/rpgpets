...
public enum RPGPetsItem {
    ...
    PET(() -> RPGPetsItem.getPetCarrier(PetType.getRandomPetType(), ConfigMessages.ITEM_PET_DEFAULTNAME.get(), 0, 0, 1, false));
     ...
     */
    public static ItemStack getPetCarrier(PetType type, String name, int level, float exp, float requiredExp, boolean grownUp) {
        ...
        lore.add(RPGPetsItem.encode(type, level, exp, grownUp));
     ...
     */
    public static String encode(PetType type, int level, float exp, boolean grownUp) {
        return ChatColor.BLACK.toString() + ChatColor.MAGIC + Double.toString(Math.random()).substring(2)
                + ":" + type.name() + ":" + level + ":" + exp + ":" + (grownUp ? 1 : 0);
    }
    ...
    public static PetDescriptor decode(ItemStack itemStack, Player owner) {
        ...
        if ((lore.size() != 5 && lore.size() != 6) || !(lore.get(4).startsWith(ChatColor.BLACK.toString() + ChatColor.MAGIC)
                || lore.get(5).startsWith(ChatColor.BLACK.toString() + ChatColor.MAGIC)))
            return null;
        ...
        if (split.length == 6)
            petDescriptor.setEntityId(Integer.parseInt(split[5]));
    ...
    public static ItemStack encodeSpawned(ItemStack itemStack, int entityId) {
        ...
        lore.add(lore.get(lore.size() - 1) + ":" + entityId);
        lore.set(lore.size() - 2, ConfigMessages.ITEM_PET_LORE_SPAWNED.get());
    ...
    /**
     * Removes the spawned status from a pet carrier
     *
     * @param stack The item to modify
     * @return The modified item
     */
    public static ItemStack removeSpawned(ItemStack stack) {
        ...
        lore.set(lore.size() - 1, line.substring(line.lastIndexOf(':') + 1));
